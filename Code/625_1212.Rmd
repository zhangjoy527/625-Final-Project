...
```{r}
Fisher <- matrix(nrow = ncol(dat), ncol = ncol(dat))
...
for (i in 1:ncol(dat)) {
  for (j in 1:ncol(dat)) {
    ...
    if (i < j) {
      ...
      result <- fisher.test(as.data.frame(dat)[,i], as.data.frame(dat)[,j],
...
```{r}
influence_info <- influence.measures(model_full)
